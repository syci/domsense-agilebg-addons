...
class purchase_order_line_master(osv.osv):
    ...
    _inherit = "purchase.order.line.master"
    _columns = {
        "product_tmpl_id": fields.many2one('product.template', "Product Template"),
        'variant_line_ids': fields.one2many('product.select.variant.line', 'master_po_line_id', 'Variants'),
        }
    ...
    def generate_detailed_lines(self, cr, uid, ids, context=None):
        res = super(purchase_order_line_master, self).generate_detailed_lines(
...
class product_select_variant_line(osv.osv):
    ...
    _columns = {
        'master_po_line_id': fields.many2one('purchase.order.line.master', 'Master PO line'),
        }
